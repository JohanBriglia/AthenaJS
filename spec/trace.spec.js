...
describe("Trace", () => {
	...
	it("averages fluencies and concatenate echoes", () => {
	    let modality = jasmine.createSpyObj("modality", ["inject", "getLength", "getMatricialLength"]);
	    modality.inject.and.returnValue({ fluency: [0.5], echo: [[0.3]] });
	    modality.getLength.and.returnValue(1);

	    let athena = jasmine.createSpyObj("athena", ["inject", "getLength", "getMatricialLength"]);
	    ...
	    expect(results.similarity).toEqual([0.25]);
	...
	it("makes a list of modalities from the probe", () => {
	    ...
	    expect(modalities[0]._modality).toEqual(probe[0]);
	    expect(modalities[0]._position).toBe(0);
	    
	    expect(modalities[1].constructor).toBe(Modality);
	    expect(modalities[1]._modality).toEqual(probe[1]);
	...
	it("makes Modalities", () => {
	    ...
	    expect(subAthenaModalities[0]._modality).toEqual([-0.5]);
	    expect(subAthenaModalities[1]._modality).toEqual([-0.25]);

	    expect(modalities[1].constructor).toBe(Modality);
	    expect(modalities[1]._modality).toEqual([0.25]);
	});
    });
});
